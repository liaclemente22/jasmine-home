...
export default function RentalsPage() {
    return (
        <Suspense fallback={<RentalsShell />}>
            <RentalsContent />
        </Suspense>
    );
}
...
function RentalsContent() {
    ...
    useEffect(() => {
        ...
        const loadProperties = async () => {
            try {
                const res = await fetch("/api/properties?type=rent");
                const data = await res.json();

                if (!ignore && data.success) {
                    setRentalProperties(data.properties);
                }
            } finally {
                if (!ignore) {
                    setLoading(false);
    ...
    const locationOptions = useMemo(
        () => [...new Set(rentalProperties.map((property) => property.location))].sort(),
        [rentalProperties]
    );
                    ...
                    <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
                        <div className="max-w-2xl">
                            ...
                            <h1 className="mt-5 font-serif text-4xl leading-tight text-[var(--textDark)] md:text-6xl">
                                Refined rental homes for calmer city living.
                            </h1>
                            <p className="mt-6 max-w-xl text-base leading-8 text-[var(--muted)] md:text-lg">
                                Explore curated condos and houses with filters that help you narrow
                                by place, budget, and space without losing the warmth of a guided experience.
                            </p>
                        </div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:w-[24rem]">
                            ...
                            <div className="rounded-[1.5rem] bg-white/78 p-5">
                                ...
                                <p className="mt-3 text-sm leading-7 text-[var(--muted)]">
                                    {searchQuery ? `Results for "${searchQuery}"` : "All active rental listings"}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <section className="mt-10 rounded-[2rem] border border-[#e9dfd3] bg-white p-6 shadow-[0_18px_40px_rgba(87,74,60,0.06)] md:p-8">
                    <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                        <div>
                            ...
                            <p className="mt-3 text-sm leading-7 text-[var(--muted)]">
                                Fine-tune by location, bedroom count, and budget.
                            </p>
                        </div>
                    ...
                    {loading ? (
                        ...
                        <div className="rounded-[2rem] border border-[#e9dfd3] bg-white px-6 py-12 text-center shadow-[0_18px_40px_rgba(87,74,60,0.05)]">
                            ...
                            <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-[var(--muted)]">
                                Try widening your location or budget range, or reset the filters to
